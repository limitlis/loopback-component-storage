...
function StorageService(options) {
   ...
   * options: {
   *    accessScopes: {
   *        method_name: ['scope_name', 'secondary_scope']
   *    }
   * }
   */
  // Check for accessScopes in options
  // if Set these will override the accessScopes.

  if (options && options.accessScopes) {
    for (var method in options.accessScopes) {
      if (StorageService.prototype[method]) {
        // console.log('Extending method %s with settings:', method, options.accessScopes[method] )
        StorageService.prototype[method].accessScopes = options.accessScopes[method];
      }
    }
  }
}
